...
# It enables the use of the decorator "@abstractmethod"
import abc

# Iterable is an abstract class with __iter__ method
# It is usefull to check wheter an object is iterable (i.e., provides and implementation for __iter__)
from collections.abc import Iterable  # pylint: disable=g-importing-member

# Module for higher-order functions: functions that act on or return other functions
# It is used for functool.partial()
...
# Library for logging in Python by Abseil
from absl import logging as absl_logging

# Gin provides a lightweight configuration framework for Python, based on dependency injection (i.e., a technique in which an object receives other objects that it depends on)
# Functions or classes can be decorated with @gin.configurable, allowing default parameter values to be supplied from a config file (or passed via the command line)
# Gin is particularly well suited for machine learning experiments, which tend to have many parameters, often nested in complex ways
import gin

# Composable transformations of Python+NumPy programs: differentiate, vectorize, JIT to GPU/TPU, and more
import jax

# Module with all the primitive and elementary operations of JAX that we already know how to transform (LAX is an anagram of XLA)
from jax import lax

# JAX random implementation with a purely functional interface
from jax import random

# JAX's Numpy on steroids
import jax.numpy as jnp

# Standard Numpy (without steroids)
import numpy as onp



# Nonlinear mappings encoding different attention kernels
# Register external JAX function with a name in Gin, and automatically make all of its parameters configurable
# For example, in .gin configuration file there would be jcos.param=value
...
def make_fast_generalized_attention(qkv_dim,
  ...
  absl_logging.info('Fast generalized attention.: %s features and renormalize=%s',
  ...
  def dot_product_attention(self,
    ...
    if self.unidirectional:
      ...
      if not self.renormalize_attention:
        ...
        thick_all_ones = jnp.zeros(key.shape[0:-1], dtype) + jnp.ones(
            key_extra.shape[0:len(axis)], dtype)
