...
from utills.T5X_utils.models import set_hardware_bernoulli, TransformerConfig, Transformer
from utills.T5X_utils.load_pytorch_weights import load_weights_from_pytorch
from utills.T5X_utils.train_pred_utils import get_configs, get_initial_params, get_optimizer, decode_tokens,get_p_pred_step, predict_output
import csv
...
def train(model_dir=None,
          data_dir=None,
          config=t5_small_glue.get_config()):
  """
  Args:
  ...
  for epoch in range(first_epoch, first_epoch + CFG.num_epochs):
    ...
    for task in eval_cache.tasks:
      ...
      predicted_tokens, predicted_text = predict_output(
      ...
      if jax.host_id() == 0:
        ...
        predictions = [
            ...
            for p, ex in zip(predicted_text, eval_cache.examples[task.name])
        ]
