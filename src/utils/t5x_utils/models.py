...
class Encoder1DBlock(nn.Module):
  ...
  def __call__(self, inputs, encoder_mask=None):
    ...
    x = SelfAttention(
        ...
        attention_fn=cfg.attention_fn)(x, encoder_mask, encoder_bias) # <---
...
class EncoderDecoder1DBlock(nn.Module):
  ...
  def __call__(self,
    ...
    x = SelfAttention(
        ...
        deterministic=cfg.deterministic,
        decode=cfg.decode,
        attention_fn=cfg.attention_fn)(x, decoder_mask, decoder_bias) # <---
    x = nn.Dropout(rate=cfg.dropout_rate)(
        x, deterministic=cfg.deterministic, broadcast_dims=(-2,))
    x = x + targets
    ...
    if encoded is None:
      ...
      y = MultiHeadDotProductAttention(
          ...
          dropout_rate=cfg.attention_dropout_rate,
          deterministic=cfg.deterministic,
          attention_fn=cfg.attention_fn)(y, encoded, encoder_decoder_mask,  # <---
